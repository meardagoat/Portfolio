...
const About = () => {
  ...
  return (
    <section id="apropos" className="py-20 sm:py-32 bg-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-12">
        ...
        >
          <div className="flex items-center gap-4 mb-6 sm:mb-8">
            ...
            <span className="text-xs sm:text-sm text-gray-500">ABOUT</span>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-12 sm:gap-16 mb-12 sm:mb-20">
          <motion.div
            ...
            transition={{ duration: 0.6 }}
          >
            <h3 className="text-xl sm:text-2xl font-medium mb-6 sm:mb-8 text-gray-900">Langues</h3>
            <div className="space-y-4 sm:space-y-6">
              {languages.map((lang, index) => (
                <div key={index}>
                  <div className="flex justify-between mb-2 sm:mb-3">
                    <span className="font-medium text-gray-900 text-sm sm:text-base">{lang.name}</span>
                    <span className="text-gray-500 text-xs sm:text-sm">{lang.level}</span>
                  </div>
          ...
          <motion.div
            ...
            transition={{ duration: 0.6 }}
          >
            <h3 className="text-xl sm:text-2xl font-medium mb-6 sm:mb-8 text-gray-900">Récompenses</h3>
              ...
              {awards.map((award, index) => (
                ...
                >
                  <div className="flex items-start gap-3 sm:gap-4">
                    <span className="text-xs sm:text-sm text-gray-500 font-medium min-w-[50px] sm:min-w-[60px]">{award.year}</span>
                    <p className="text-gray-700 text-sm sm:text-base">{award.title}</p>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        <motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          <h3 className="text-xl sm:text-2xl font-medium mb-8 sm:mb-12 text-gray-900">Centres d'intérêt</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
            {interests.map((interest, index) => (
              <motion.div
                key={index}
                className="border border-gray-200 p-6 sm:p-8 text-center hover:border-gray-900 transition-colors"
              ...
              >
                <motion.div
                  className="text-3xl sm:text-4xl text-gray-900 mb-3 sm:mb-4 flex justify-center"
                  whileHover={{ rotate: 360, scale: 1.1 }}
                  transition={{ duration: 0.5 }}
                >
                  {interest.icon}
                </motion.div>
                <p className="font-medium text-gray-700 text-sm sm:text-base">{interest.name}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
